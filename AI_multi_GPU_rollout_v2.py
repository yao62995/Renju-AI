...
from AI_import import patterns
...
def inference(_input, features=64):
    fc1 = full_connect(_input, (features, 1024), "fc_1")
    fc_n = full_connect(fc1, (1024, 512), "fc_n")

    with tf.variable_scope("out") as scope:
        weights = _variable_with_weight_decay('weights', shape=(512, board_size * board_size),
                                              stddev=0.04, wd=0.004)
        biases = _variable_on_cpu('biases', [board_size * board_size], tf.constant_initializer(0.1))
        softmax_linear = tf.add(tf.matmul(fc_n, weights), biases, name=scope.name)
...
def train(epochs=200, predict=False):
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        num_batchs_per_epochs = int(patterns.num_batchs_per_epochs(BATCH_SIZE))
        ...
        for i in xrange(gpu_num):
            with tf.device('/gpu:%d' % i):
                with tf.name_scope('%s_%d' % (TOWER_NAME, i)) as scope:
                    # all towers.
                    batch_input = tf.placeholder(tf.float32, [None, features])
        ...
        while epochs_step <= (global_epoch + epochs):
            ...
            for idx in xrange(gpu_num):
                samples = patterns.next_fetch_rows(BATCH_SIZE)
                feature = np.array([sample[0] for sample in samples], dtype=np.float32)
