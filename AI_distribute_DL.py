...
def tower_loss(scope, data_dir):
    ...
    """
    file_records = "/home/yaojian-xy/%s/renju_planes.tfrecords" % data_dir
    states, labels = distorted_inputs(file_records=file_records, shape=(15, 15, 3), batch_size=128,
                                      num_epochs=200, num_threads=5, num_examples_per_epoch=128000)
...
def create_dataset():
    from network.transform import convert_images
    batch_size = 128
    writer = tf.python_io.TFRecordWriter("./data/renju_planes.tfrecords")
    for idx, samples in enumerate(corpus.iterator_fetch_rows(batch_size)):
        states = [sample[0].get_states() for sample in samples]
        labels = [sample[1] for sample in samples]
        convert_images(states, labels, writer)
        if idx % 100 == 0:
            print "finish step:", idx
    writer.close()
