function Game(boardElm, boardBackgroundElm) {
    ...
    this.latest_go = function () {
        var move = history[history.length - 1];
        return move["r"] * 15 + move["c"];
    };
    ...
    this.undo = function () {
        ...
        var ai_player = players['white'];
        if(players['black'] instanceof AIPlayer) {
            ai_player = players['black'];
        }
        //ai_player.undo();
    };
    ...
    this.win = function () {
        ...
        board.setClickable(false);
        showWinDialog(this);
        var url = "http://dpl04.skyeyes.lycc.qihoo.net:2220/operate?handle=release&auth=" + this.auth_name;
        this.auth_name = "";
        $.ajax({
            type: "GET",
            url: url,
