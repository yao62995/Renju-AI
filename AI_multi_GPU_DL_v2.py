...
train_dir = "./policy_dl_multi_gpu_models/"
...
def train(epochs=200, predict=False):
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        while epochs_step <= (global_epoch + epochs):
            ...
            _, loss_value, acc_value, global_step_val, learn_rating = sess.run([train_op, loss, accuracy, global_step, lr],
                                                                 feed_dict=feeds)
            ...
            if global_step_val % 10 == 0:
                logger.info("train policy dl multi_GPU network, epoch=%d, step=%d, loss=%.6f, acc=%.6f, lr=%.6f, time=%d(ms)" % (
            ...
            if step > num_batchs_per_epochs:
                ...
                logger.info("train policy dl multi_GPU network, epochs=%d, average_loss=%.7f, average_acc=%.7f" %
                ...
                if epochs_step % 3 == 0:
                    ...
                    logger.info("save policy dl multi_GPU model: %s" % filename)
